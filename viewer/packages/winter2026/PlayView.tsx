...
export function PlayView({ bots }: PlayViewProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const statusLine = `seed=${actualSeed}  ${map.width}\u00d7${map.height}  winner=${winnerStr}  score=${runData.score_blue}:${runData.score_red}  turns=${runData.turns}  blue ttfo=${ttfo[0].toFixed(0)}ms aot=${aot[0].toFixed(0)}ms  red ttfo=${ttfo[1].toFixed(0)}ms aot=${aot[1].toFixed(0)}ms`
