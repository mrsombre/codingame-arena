...
import { lerp, lerpColor, unlerp, unlerpUnclamped, } from '../core/utils.js';
import { parseData, parseGlobalData } from './Deserializer.js';
import { TooltipManager } from './TooltipManager.js';
import ev from './events.js';
import { choice, fit, keyOf, last, setAnimationProgress, } from './utils.js';
import { AVATAR_RECT, CONVEYOR_HEIGHT, CONVEYOR_SCALE, CONVEYOR_WIDTH, GAME_ZONE_RECT, GRASS_FRAMES, BRUSH_FRAMES, HUD_COLORS, MESSAGE_RECT, MOUNTAIN_FRAMES_1x1, MOUNTAIN_FRAMES_2x2, MOUNTAIN_FRAMES_2x1, NAME_RECT, POI_FRAMES, RAIL_DATA, SCORE_RECT, TOWN_FRAME, TRAIN_FRAME, ZONE_LINE_WIDTH, INK_FRAMES, } from './assetConstants.js';
...
export class ViewModule {
    ...
    createEffect(type) {
        ...
        if (type === 'train') {
            ...
            const effect = { busy: false, display, sprite, overlay: null, scaler };
            return effect;
        }
    ...
    zoneStateAt(zoneId, progress) {
        let zone = Object.assign(Object.assign({}, this.previousData.zones[zoneId]), { prevInstability: this.previousData.zones[zoneId].instability, disruptP: 1, inkP: 1, prevInked: this.previousData.zones[zoneId].inked });
        for (const event of this.currentData.events) {
            if (event.zoneId === zone.id) {
                if (event.type === ev.INK) {
                    ...
                    if (p >= 0) {
                        zone = Object.assign(Object.assign({}, zone), { inked: true, inkP: Math.min(1, p) });
                    }
                }
                else if (event.type === ev.DISRUPT) {
                    const p = this.getAnimProgress(event.animData, progress);
                    if (p >= 0) {
                        zone = Object.assign(Object.assign({}, zone), { instability: zone.instability + 1, disruptP: Math.min(1, p) });
                    }
                }
            }
        }
        ...
        for (const zone of allZones) {
            ...
            for (const cell of allCells) {
                ...
                for (const d of this.drawData) {
                    ...
                    if (!zoneSet.has(key)) {
                        const gradient = this.getFromPool('gradient');
                ...
                for (const d of this.cornerData) {
                    ...
                    const key3 = `${x + d.directions[0].x + d.directions[1].x},${y + d.directions[0].y + d.directions[1].y}`;
                    if (zoneSet.has(key1) && zoneSet.has(key2) && !zoneSet.has(key3)) {
                        const gradient = this.getFromPool('gradient_corner');
                        gradient.display.width = ZONE_GRADIENT_LENGTH;
    ...
    alphaFromInstability(instability) {
    ...
    updateDisrupts() {
        ...
        for (const e of disruptEvents) {
            ...
            if (p > 0 && p < 1) {
                ...
                const { display: inkFx } = this.getFromPool(`ink_${pIdx}`);
    ...
    updateConveyors() {
        ...
        for (const lcta of this.liveConnectionsToAnimate) {
            ...
            display.width = Math.sqrt(Math.pow((toPos.x - fromPos.x), 2) + Math.pow((toPos.y - fromPos.y), 2));
    ...
    updateGrid() {
        ...
        for (const e of inkEvents) {
            ...
            if (p > 0 && p < 1) {
                ...
                const center = this.getZoneCenter(e.zoneId);
                this.inkAnim.drawCircle(center.x * this.tileSize + this.tileSize / 2, center.y * this.tileSize + this.tileSize / 2, p * 4 * this.tileSize);
            }
        }
    ...
    placeInGameZone(display, coord) {
        ...
        display.position.set(pos.x + this.tileSize / 2, pos.y + this.tileSize / 2);
    }
    updateTrains() {
        ...
        this.currentData.events
            ...
            if (p >= 1) {
                ...
                activeConnectionMap[keyOf(c.fromTownId, c.toTownId)] = Object.assign(Object.assign({}, c), { fadeOutP: Math.max(0, p) });
            }
        });
        ...
        for (const key in activeConnectionMap) {
            ...
            if (api.options.connectionDisplay === 0) {
                ...
                for (let i = 0; i < trainCount; i++) {
                    let effect = this.getFromPool('train');
            ...
            this.liveConnectionsToAnimate.push(Object.assign(Object.assign({}, connection), { trainEffects }));
        }
    }
    tintTrack(sprite, pId) {
        if (pId === 2) {
    ...
    updateTracks() {
        ...
        for (const key of railsToDraw) {
            ...
            const tileTrack = this.getTileTrackAt({ x, y }, this.progress);
            const effect = this.getFromPool('track');
            ...
            this.drawData
                ...
                if (nCoord.x >= 0 &&
                    ...
                    if (nTileTrack > -1 ||
                        this.globalData.towns.some((t) => t.coord.x === nCoord.x && t.coord.y === nCoord.y)) {
        ...
        buildEvents
            ...
            if (p > 0 && p < 1) {
                ...
                for (const pIdx of brushPlayers) {
                    const { display: brushFx } = this.getFromPool(`brush_${pIdx}`);
    ...
    renderLiveConnections() {
        ...
        for (const liveConnection of this.liveConnectionsToAnimate) {
            ...
            for (let i = 0; i < trainCount; i++) {
                ...
                const tripP = unlerp(0, totalJourneyTime, localTime % totalJourneyTime);
                const p = tripP;
    ...
    reinitScene(container, canvasData) {
        ...
        container.on('mousemove', (event) => {
            ...
            this.hoverTownId = (_b = (_a = this.globalData.townMap[`${x},${y}`]) === null || _a === void 0 ? void 0 : _a.id) !== null && _b !== void 0 ? _b : null;
        });
        this.tooltipManager.registerGlobal((data) => {
            ...
            const town = this.globalData.towns.find((t) => t.coord.x === x && t.coord.y === y);
            if (town) {
                let townBlock = `Town ${town.id}\nDesired: ${town.desiredConnections.length
    ...
    initZoneLines(layer) {
        ...
        const alreadyDrawnBorders = new Set();
    ...
    initGrid(layer) {
        this.tiles = [];
        let poiIdx = 0;
        const ignoredMountains = new Set();
        for (let y = 0; y < this.globalData.height; y++) {
            const row = [];
            for (let x = 0; x < this.globalData.width; x++) {
                const tile = this.globalData.tiles[this.flat({ x, y })];
                ...
                if (ignoredMountains.has(`${x},${y}`)) {
                    ...
                    if ((rightCell === null || rightCell === void 0 ? void 0 : rightCell.type) === TYPE_MOUNTAIN && (bottomCell === null || bottomCell === void 0 ? void 0 : bottomCell.type) === TYPE_MOUNTAIN && (bottomRightCell === null || bottomRightCell === void 0 ? void 0 : bottomRightCell.type) === TYPE_MOUNTAIN) {
                        // 2x2
                    ...
                    else if ((rightCell === null || rightCell === void 0 ? void 0 : rightCell.type) === TYPE_MOUNTAIN) {
                        // 2x1
                        ignoredMountains.add(`${x + 1},${y}`);
    ...
    handleGlobalData(players, raw) {
        ...
        this.globalData = Object.assign(Object.assign({}, globalData), { players: players, playerCount: players.length, townMap: globalData.towns.reduce((acc, town) => {
                acc[`${town.coord.x},${town.coord.y}`] = town.id;
                return acc;
            }, {}) });
    }
    trackStateAt(coord, progress) {
    }
    handleFrameData(frameInfo, raw) {
        var _a, _b, _c;
        const dto = parseData(raw, this.globalData);
        const previousFrame = last(this.states);
        const frameData = Object.assign(Object.assign({}, dto), { previous: null, frameInfo, connections: (_a = previousFrame === null || previousFrame === void 0 ? void 0 : previousFrame.connections) !== null && _a !== void 0 ? _a : [], zones: (_b = previousFrame === null || previousFrame === void 0 ? void 0 : previousFrame.zones) !== null && _b !== void 0 ? _b : this.globalData.zones.map((z) => (Object.assign(Object.assign({}, z), { inked: false, instability: 0 }))), tiles: (_c = previousFrame === null || previousFrame === void 0 ? void 0 : previousFrame.tiles) !== null && _c !== void 0 ? _c : new Array(this.globalData.width * this.globalData.height).fill({
                track: -1,
            }), players: previousFrame != null
                ? previousFrame.players.map((v) => (Object.assign({}, v)))
                : this.globalData.players.map((p) => ({
                    score: 0,
                })) });
        frameData.previous = previousFrame !== null && previousFrame !== void 0 ? previousFrame : frameData;
        frameData.tiles = frameData.tiles.map(t => (Object.assign(Object.assign({}, t), { history: [] })));
        ...
        for (const event of dto.events) {
            if (event.type === ev.BUILD) {
                ...
                frameData.zones[event.zoneId] = Object.assign(Object.assign({}, frameData.zones[event.zoneId]), { instability: frameData.zones[event.zoneId].instability + 1 });
            }
            else if (event.type === ev.EARN_POINTS) {
                frameData.players = frameData.players.map((p, idx) => {
                    if (idx === event.playerIdx) {
                        return Object.assign(Object.assign({}, p), { score: p.score + event.score });
                    }
                    return p;
                });
            }
                ...
                frameData.zones = [...frameData.zones];
                frameData.zones[event.zoneId] = Object.assign(Object.assign({}, frameData.zones[event.zoneId]), { inked: true });
                ...
                frameData.connections = [...frameData.connections];
                const idx = frameData.connections.findIndex((c) => c.fromTownId === event.fromTownId && c.toTownId === event.toTownId);
