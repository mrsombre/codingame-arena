...
Statement as published for **League 1**. Later leagues unlock disruption
(`DISRUPT`) and the `instability` / `inked` cell fields, which are sent but
documented as unused here. Sections below the horizontal rule are derived from
the local engine dump, not from the contest page.

## Goal

The first two leagues have a special objective to achieve. The full game will
play, but you can only win by completing the objective. Once you do, you can
start work on your complete bot.

> 🎯 **League Objective 1:** Connect two towns to form an active train
> connection to instantly win the game.

The Boss AI will skip their turns. If you fail to form a connection within 100
turns, you will lose. Win at least 3 or more times out of 5 to progress to the
next league.
...
disruption by players — more information on disruption in the next league.
...
- Form an active connection and score any number of points.

### ⛔ Defeat conditions

- Your program does not provide a command in the allotted time, or one of the
  commands is invalid.
- You do not complete the objective within 100 turns.
...
- **Next `height * width` lines:** state of each cell, in the same order they
  ...
  - `instability` — unused in this league.
  - `inked` — unused in this league.
  - `partOfActiveConnections` (not useful in this league) — a string of
    comma-separated `townId` pairs indicating this cell is part of an active
    connection between those two towns, e.g. `1-2,1-3,4-7`. `x` if this cell is
    not part of any active connection.
...
- **`DISRUPT` accepts two forms.** `DISRUPT zoneId` and `DISRUPT x y` (which
  ...
  the statement introduces them in a later league.
- **Invalid actions do not always disqualify.** An unparseable command
  ...
  interrupts the remainder of that autobuild.
