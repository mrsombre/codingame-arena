package engine

import (
	"strings"
	"testing"

	"github.com/stretchr/testify/assert"

	"github.com/mrsombre/codingame-arena/internal/util/sha1prng"
)

// Seed-parity tests for GridMaker. Mirrors
// games/spring2026/engine/engine_board_seed_test.go and
// games/spring2020/engine/spring2020_maps_tetris_based_map_generator_test.go.
//
// The compared artefact is the global-info block minus its leading
// player-index line, which is per-player rather than per-map: grid width,
// grid height, one `regionId type` line per cell in row-major order, the town
// count, then one `id x y desiredConnections` line per town. That is the
// whole of what GridMaker decides and the whole of what a bot sees of it.

const (
	testArenaPositiveSeed = int64(468706172918629800)
	testArenaNegativeSeed = int64(-468706172918629800)
)

// testArenaExpectedGlobalInfoPositive is captured from a live upstream match
// at the positive reference seed. Acceptance bar: the engine must reproduce
// this byte-for-byte.
var testArenaExpectedGlobalInfoPositive = strings.Join([]string{
	"21",
	"14",
	"0 0",
	"0 0",
	"0 0",
	"1 0",
	"1 0",
	"1 0",
	"2 0",
	"2 1",
	"2 0",
	"2 0",
	"3 0",
	"3 0",
	"3 0",
	"4 0",
	"4 0",
	"5 0",
	"5 0",
	"5 0",
	"5 0",
	"6 0",
	"6 0",
	"0 0",
	"0 0",
	"0 0",
	"1 0",
	"1 0",
	"1 0",
	"2 1",
	"2 1",
	"2 0",
	"3 0",
	"3 0",
	"3 0",
	"4 0",
	"4 0",
	"4 0",
	"5 0",
	"5 0",
	"12 0",
	"6 0",
	"6 0",
	"6 0",
	"7 0",
	"7 0",
	"7 0",
	"1 0",
	"1 0",
	"8 1",
	"9 1",
	"9 0",
	"2 0",
	"3 0",
	"10 0",
	"3 0",
	"4 0",
	"11 0",
	"4 0",
	"4 0",
	"5 0",
	"12 0",
	"13 0",
	"6 0",
	"6 0",
	"7 0",
	"7 0",
	"7 0",
	"8 0",
	"8 0",
	"8 1",
	"9 0",
	"9 0",
	"9 0",
	"10 0",
	"10 0",
	"10 0",
	"10 0",
	"11 0",
	"11 0",
	"12 0",
	"12 0",
	"12 0",
	"13 0",
	"13 0",
	"13 0",
	"14 0",
	"14 0",
	"14 0",
	"8 0",
	"8 0",
	"8 1",
	"9 0",
	"9 0",
	"10 0",
	"10 0",
	"17 0",
	"17 0",
	"11 0",
	"11 0",
	"11 0",
	"11 0",
	"12 0",
	"19 0",
	"13 0",
	"13 0",
	"20 0",
	"14 0",
	"14 0",
	"15 0",
	"15 0",
	"15 1",
	"15 1",
	"16 1",
	"16 0",
	"16 0",
	"16 0",
	"17 0",
	"17 0",
	"17 0",
	"18 0",
	"18 0",
	"11 0",
	"19 0",
	"19 0",
	"20 0",
	"20 0",
	"20 0",
	"14 0",
	"14 0",
	"21 0",
	"15 0",
	"15 1",
	"15 0",
	"16 1",
	"16 0",
	"23 1",
	"16 1",
	"24 1",
	"17 0",
	"17 0",
	"18 0",
	"18 0",
	"26 0",
	"19 0",
	"19 0",
	"19 0",
	"20 0",
	"20 0",
	"14 0",
	"21 0",
	"21 0",
	"21 1",
	"21 1",
	"15 0",
	"16 1",
	"23 1",
	"23 1",
	"24 0",
	"24 1",
	"17 0",
	"17 0",
	"18 0",
	"18 0",
	"26 2",
	"26 0",
	"19 0",
	"27 0",
	"27 0",
	"27 0",
	"28 1",
	"21 1",
	"21 1",
	"22 1",
	"22 0",
	"22 0",
	"29 0",
	"23 0",
	"23 0",
	"24 0",
	"24 1",
	"24 0",
	"24 0",
	"25 0",
	"18 2",
	"26 2",
	"26 2",
	"33 0",
	"27 0",
	"27 0",
	"27 0",
	"28 0",
	"28 0",
	"28 0",
	"22 0",
	"22 0",
	"29 0",
	"29 0",
	"23 0",
	"23 1",
	"24 1",
	"31 1",
	"25 0",
	"25 0",
	"25 0",
	"25 0",
	"26 2",
	"26 2",
	"33 0",
	"33 0",
	"27 0",
	"34 0",
	"28 0",
	"28 0",
	"28 1",
	"22 1",
	"29 1",
	"29 1",
	"30 1",
	"30 1",
	"31 1",
	"31 0",
	"31 0",
	"25 0",
	"32 0",
	"32 0",
	"32 0",
	"33 0",
	"33 0",
	"33 0",
	"34 0",
	"34 0",
	"34 0",
	"35 2",
	"35 1",
	"36 1",
	"36 0",
	"29 2",
	"29 2",
	"30 0",
	"30 2",
	"38 2",
	"38 0",
	"31 0",
	"31 0",
	"32 0",
	"32 0",
	"32 0",
	"40 0",
	"33 0",
	"40 0",
	"34 0",
	"41 0",
	"41 0",
	"35 2",
	"35 2",
	"35 0",
	"36 0",
	"36 0",
	"36 2",
	"30 2",
	"37 2",
	"37 2",
	"38 2",
	"38 0",
	"31 0",
	"39 0",
	"39 0",
	"39 0",
	"40 0",
	"40 0",
	"40 0",
	"34 0",
	"41 0",
	"41 0",
	"35 0",
	"35 2",
	"35 2",
	"36 0",
	"36 2",
	"30 2",
	"30 0",
	"37 0",
	"37 0",
	"37 0",
	"38 0",
	"38 0",
	"39 0",
	"39 0",
	"39 0",
	"39 0",
	"40 0",
	"40 0",
	"34 0",
	"41 0",
	"41 0",
	"5",
	"0 8 1 3",
	"1 1 2 0,2",
	"2 19 6 0",
	"3 15 6 1,2,4",
	"4 6 11 0,1,2",
}, "\n")

// testArenaExpectedGlobalInfoNegative is the same artefact for the negative
// reference seed. Same format as the positive case.
var testArenaExpectedGlobalInfoNegative = strings.Join([]string{
	"24",
	"16",
	"0 0",
	"0 0",
	"0 0",
	"0 0",
	"1 2",
	"1 2",
	"1 2",
	"1 0",
	"2 1",
	"2 0",
	"3 0",
	"3 0",
	"3 0",
	"4 0",
	"4 0",
	"4 0",
	"4 0",
	"5 0",
	"5 0",
	"5 0",
	"6 0",
	"6 0",
	"6 0",
	"6 0",
	"0 0",
	"0 0",
	"0 0",
	"7 2",
	"1 2",
	"1 0",
	"1 2",
	"1 0",
	"2 1",
	"2 1",
	"3 0",
	"3 1",
	"3 1",
	"4 1",
	"4 0",
	"4 0",
	"4 0",
	"5 0",
	"5 0",
	"5 0",
	"6 0",
	"6 0",
	"6 0",
	"6 0",
	"0 0",
	"0 0",
	"7 0",
	"7 0",
	"1 0",
	"1 0",
	"2 2",
	"2 2",
	"2 0",
	"2 1",
	"9 1",
	"3 1",
	"3 0",
	"3 1",
	"4 0",
	"4 0",
	"5 0",
	"5 0",
	"12 0",
	"12 0",
	"12 0",
	"13 0",
	"13 0",
	"13 0",
	"7 0",
	"7 0",
	"7 0",
	"7 0",
	"8 0",
	"8 0",
	"9 0",
	"9 0",
	"9 2",
	"9 2",
	"9 0",
	"10 0",
	"10 0",
	"10 1",
	"10 0",
	"11 0",
	"11 0",
	"11 0",
	"12 0",
	"12 0",
	"12 0",
	"13 0",
	"13 0",
	"13 0",
	"14 0",
	"14 0",
	"14 0",
	"7 0",
	"8 0",
	"8 0",
	"8 0",
	"8 0",
	"16 2",
	"9 1",
	"10 1",
	"10 1",
	"10 0",
	"17 1",
	"18 0",
	"11 0",
	"11 0",
	"11 0",
	"12 0",
	"12 0",
	"12 0",
	"13 0",
	"13 0",
	"13 0",
	"14 0",
	"14 0",
	"7 0",
	"7 0",
	"15 0",
	"15 0",
	"15 0",
	"8 0",
	"16 2",
	"16 1",
	"10 0",
	"17 1",
	"17 0",
	"17 1",
	"18 0",
	"18 0",
	"11 0",
	"19 0",
	"19 0",
	"19 0",
	"19 2",
	"20 0",
	"20 0",
	"20 0",
	"14 1",
	"14 1",
	"14 1",
	"15 0",
	"15 0",
	"15 0",
	"16 0",
	"16 0",
	"16 2",
	"16 1",
	"17 0",
	"17 1",
	"17 1",
	"18 1",
	"18 0",
	"18 0",
	"19 0",
	"19 0",
	"19 0",
	"19 2",
	"27 2",
	"20 0",
	"20 2",
	"20 0",
	"14 0",
	"21 0",
	"21 1",
	"15 1",
	"15 1",
	"15 0",
	"16 2",
	"23 2",
	"23 2",
	"23 1",
	"23 0",
	"24 0",
	"17 0",
	"18 0",
	"18 0",
	"25 0",
	"19 0",
	"26 2",
	"26 0",
	"26 0",
	"27 2",
	"27 2",
	"20 2",
	"20 0",
	"21 0",
	"21 0",
	"21 2",
	"22 2",
	"22 1",
	"22 0",
	"22 0",
	"23 2",
	"23 0",
	"23 1",
	"23 1",
	"24 1",
	"24 1",
	"25 0",
	"25 0",
	"25 0",
	"26 2",
	"26 2",
	"26 2",
	"26 2",
	"27 2",
	"27 2",
	"27 0",
	"27 0",
	"21 0",
	"21 0",
	"21 2",
	"22 2",
	"22 1",
	"22 0",
	"29 0",
	"29 0",
	"29 0",
	"30 0",
	"30 0",
	"24 0",
	"24 1",
	"31 1",
	"31 0",
	"25 0",
	"25 0",
	"26 0",
	"33 0",
	"33 0",
	"33 2",
	"27 2",
	"34 0",
	"34 0",
	"28 0",
	"28 0",
	"28 2",
	"28 2",
	"29 1",
	"29 0",
	"29 0",
	"37 0",
	"30 0",
	"30 0",
	"30 0",
	"24 0",
	"31 0",
	"31 1",
	"31 1",
	"32 0",
	"25 0",
	"33 0",
	"33 0",
	"33 2",
	"34 0",
	"34 0",
	"34 0",
	"34 0",
	"28 0",
	"28 0",
	"28 0",
	"28 0",
	"29 1",
	"36 1",
	"37 0",
	"37 0",
	"30 0",
	"30 0",
	"30 0",
	"38 0",
	"38 0",
	"31 0",
	"32 1",
	"32 1",
	"39 2",
	"40 2",
	"40 2",
	"40 2",
	"41 2",
	"41 2",
	"41 2",
	"34 2",
	"35 0",
	"35 0",
	"35 0",
	"35 0",
	"29 0",
	"36 1",
	"36 1",
	"37 0",
	"37 0",
	"37 0",
	"38 0",
	"38 0",
	"38 0",
	"38 0",
	"32 0",
	"39 1",
	"39 2",
	"40 2",
	"40 0",
	"40 0",
	"40 0",
	"41 0",
	"41 0",
	"41 0",
	"35 0",
	"35 0",
	"35 2",
	"42 2",
	"36 0",
	"36 0",
	"43 1",
	"43 1",
	"43 0",
	"37 1",
	"44 1",
	"44 1",
	"38 1",
	"32 1",
	"32 1",
	"39 1",
	"39 2",
	"39 0",
	"46 0",
	"46 0",
	"46 0",
	"41 0",
	"47 0",
	"41 0",
	"42 0",
	"42 0",
	"42 2",
	"42 2",
	"42 0",
	"36 0",
	"43 0",
	"43 1",
	"43 1",
	"44 1",
	"44 0",
	"44 0",
	"45 0",
	"32 0",
	"45 1",
	"45 2",
	"46 2",
	"46 2",
	"46 2",
	"47 0",
	"47 0",
	"47 0",
	"47 0",
	"47 0",
	"42 0",
	"42 0",
	"42 2",
	"42 0",
	"36 0",
	"36 0",
	"36 0",
	"43 0",
	"43 0",
	"44 0",
	"44 0",
	"44 0",
	"45 0",
	"45 0",
	"45 1",
	"45 0",
	"45 0",
	"45 0",
	"46 0",
	"46 0",
	"46 0",
	"47 0",
	"47 0",
	"47 0",
	"7",
	"0 15 2 x",
	"1 22 1 5",
	"2 6 6 0,1,4",
	"3 1 9 0,1,2",
	"4 8 9 0,1,3",
	"5 13 11 0,3,6",
	"6 17 13 0,1,4",
}, "\n")

// buildGlobalInfo generates a map from seed and serializes it as the engine
// hands it to a bot, dropping the player-index line so the artefact is the
// map alone.
func buildGlobalInfo(seed int64) string {
	maker := NewGridMaker()
	maker.Init(sha1prng.New(seed), false)
	game := NewGame(nil, DEFAULT_LEAGUE)
	game.Grid = maker.Make()

	return strings.Join(SerializeGlobalInfoFor(NewPlayer(0), game)[1:], "\n")
}

// TestSeedDeterminism guards determinism: the same seed must produce the same
// map every run.
func TestSeedDeterminism(t *testing.T) {
	for _, seed := range []int64{testArenaPositiveSeed, testArenaNegativeSeed} {
		assert.Equalf(t, buildGlobalInfo(seed), buildGlobalInfo(seed),
			"map generation must be deterministic for seed=%d", seed)
	}
}

// TestSeedPositiveParityCheck is the byte-for-byte reference match for the
// positive reference seed.
func TestSeedPositiveParityCheck(t *testing.T) {
	assert.Equalf(t, testArenaExpectedGlobalInfoPositive, buildGlobalInfo(testArenaPositiveSeed),
		"map parity mismatch for seed=%d", testArenaPositiveSeed)
}

// TestSeedNegativeParityCheck is the byte-for-byte reference match for the
// negative reference seed. It draws a different grid size than the positive
// one, so the pair covers both of the dimension paths makeZones can take.
func TestSeedNegativeParityCheck(t *testing.T) {
	assert.Equalf(t, testArenaExpectedGlobalInfoNegative, buildGlobalInfo(testArenaNegativeSeed),
		"map parity mismatch for seed=%d", testArenaNegativeSeed)
}
