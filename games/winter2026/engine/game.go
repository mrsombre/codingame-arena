...
func (g *Game) somethingSolidUnder(c grid.Coord, ignoreBody []grid.Coord) bool {
	below := c.AddXY(0, 1)
	if coordSliceContainsWinter(ignoreBody, below) {
		return false
	}
	if g.grid.Get(below).Type == grid.TileWall {
		return true
	...
	for _, b := range g.liveBirds() {
		if coordSliceContainsWinter(b.Body, below) {
			return true
		}
	}
	return coordSliceContainsWinter(g.grid.Apples, below)
...
func (g *Game) getIntercoiledBirds() [][]*Bird {
	intercoiledGroups := make([][]*Bird, 0)
	allBirds := make([]*Bird, 0)
	for _, b := range g.liveBirds() {
		onSolidGround := false
	...
	visited := make(map[*Bird]struct{})
	for _, bird := range allBirds {
		if _, ok := visited[bird]; ok {
			continue
		}
		group := make([]*Bird, 0)
		toVisit := []*Bird{bird}
		for len(toVisit) > 0 {
			...
			for _, other := range allBirds {
				...
				if birdsAreTouchingVertically(current, other) {
					toVisit = append(toVisit, other)
				}
			}
		}
		if len(group) > 1 {
			intercoiledGroups = append(intercoiledGroups, group)
		}
	}
...
func (g *Game) doIntercoiledFalls(fallDistances map[int]int, outOfBounds *[]*Bird) bool {
	...
	for somethingFell {
		somethingFell = false
		for _, birds := range g.getIntercoiledBirds() {
			metaBody := make([]grid.Coord, 0)
			for _, b := range birds {
...
func birdsAreTouchingVertically(bird, other *Bird) bool {
	for _, c1 := range bird.Body {
		for _, c2 := range other.Body {
			if c1.ManhattanTo(c2) == 1 && c1.X == c2.X {
				return true
			}
		}
	}
...
func (g *Game) birdByID(birdID int) *Bird {
	if bird, ok := g.birdByIDCache[birdID]; ok {
		return bird
	}
	for _, b := range g.allBirds() {
		if b.ID == birdID {
			g.birdByIDCache[birdID] = b
			return b
		}
	}
	panic("bird not found")
}
