import { EventDto, FrameDataDto, GlobalData, GlobalDataDto, GlobalTileDto, GlobalZoneDto, TileDto, TownDto } from '../types.js'
...
export function parseGlobalData (unsplit: string): GlobalDataDto {
  ...
  for (let y = 0; y < height; y++) {
    for (let x = 0; x < width; x++) {
      ...
      const tile: GlobalTileDto = {
        coord: {x, y},
        zoneId: +tokens[0],
        type: +tokens[1],
      }
      tiles.push(tile)
      zoneMap[tile.zoneId] = zoneMap[tile.zoneId] ?? {coords: [], id: tile.zoneId, owner: -1}
  ...
  return {
    ...
    zoneMap,
    passiveIncome,
    zones: Object.values<GlobalZoneDto>(zoneMap).sort((a, b) => a.id - b.id),
    towns
  }
}
