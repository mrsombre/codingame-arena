...
func SerializeUsage(fs *pflag.FlagSet) string {
  ...
  arena game serialize <game> [OPTIONS]
...
Output:
  ...
  Pipe straight into a bot binary to drive a single-turn invocation:
      arena game serialize winter2026 --seed 42 | bin/bot-winter2026-cpp

  Omitting --seed picks a fresh seed each call (current Unix nanoseconds),
  matching the default for ` + "`arena run`" + `. Pass --seed to reproduce a
  specific map.
...
Use cases:
  ...
  - Debug fog-of-war / side-specific input differences with --player=0/1.`
	return arena.CommandUsage("game serialize <game>", "Print the bot-stdin bytes (globals + turn-0 frame) for the active game.", fs, extra)
}
